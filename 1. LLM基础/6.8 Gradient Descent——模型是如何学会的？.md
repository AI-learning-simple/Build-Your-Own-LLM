# Experiment 6.8：Gradient Descent——模型是如何学会的？

## 实验目标

上一节（6.7），我们亲手算出了 Loss 对每一个 Weight 的梯度——但"知道梯度"和"用好梯度"是两回事。模型还需要知道：**Weight 应该往哪个方向调整、调整多少，才能让 Loss 变小？** 本实验将实现深度学习中最核心的算法——**Gradient Descent（梯度下降）**。
...
对于简单函数（如 `(w-3)²`），我们可以手动求导。但神经网络有成千上万个参数，手动求导几乎不可能。现代深度学习框架（PyTorch、TensorFlow）都实现了**自动求导（Automatic Differentiation）**，通过**反向传播（Backpropagation）**，从输出的 Loss 开始，沿着计算图反向逐层计算每一个参数的梯度。上一节（6.7）我们已经亲手展开过这个过程：用第4章的数字链手算了一次完整反向传播，写了一个迷你自动求导引擎，还与 PyTorch 三方对账。所以这里的梯度不再是黑盒。核心结论：
...
**这里可以回答第3、4章都留下的一个问题：Embedding 到底是不是和 Weight 一起被更新的？** 答案是：是的，没有另外一套机制——反向传播会一路从 Loss 传回 MLP，再传回 Embedding，两者都用同一条公式更新。但有一个细节值得注意：**Embedding Lookup 本质上是数组索引（6.2节讲过），所以反向传播传回来的梯度，也只会落在这一步实际被查到的那几行上**——例如这一批训练数据只用到了 `I`、`love` 两个 Token，那么也只有 Embedding Matrix 里 `I` 和 `love` 对应的那两行会拿到梯度、被更新，词表里其它没出现过的词，这一步梯度是 0，不会有任何变化。这也是为什么高频词（如 `the`）的 Embedding 通常学得更快更好：它被抽到、被更新的次数远多于生僻词。
