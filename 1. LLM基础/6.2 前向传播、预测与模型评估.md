...
**这个公式的作用**：把模型分配给真实 Target 的概率转换为单样本错误值。`p_target` 越接近 `1`，Loss 越接近 `0`；概率越接近 `0`，惩罚越大。
...
这里 `N` 是评测位置数量，`y_t` 是第 `t` 个真实 Target。这个公式的作用是先把不同位置的负 Log Probability 平均，得到语料级 Cross Entropy。

Perplexity 再把平均对数损失转换回更直观的正数尺度：

```math
\text{PPL}=\exp(\text{CE})
```

PPL 可以理解为模型在当前 Tokenizer 和数据集下的平均不确定程度；它不是实际候选数，也不能跨不同分词方案直接比较。
