...
最后再从这些概率中选择一个 Token 进行输出（怎么选择——Greedy、Top-k、Temperature 等采样策略，会在第 6.3 节《第一次预测——从 Logits 到概率分布》里亲手实现）。本章只需要记住：
...
在模型上下文窗口与能力允许的范围内，相关 Context 往往有助于预测；无关内容也可能形成干扰。模型如何聚合上下文信息，会在第 8 章 Attention 和第 9 章 Transformer Block 中继续讨论。
