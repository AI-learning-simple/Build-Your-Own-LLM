# 13. Transformer 层堆叠（Stacking Transformer）

> **本章目标**
>
> 第 11、12 章拼出了单个 Transformer 模块（Transformer Block，朴素版与现代版），但 GPT、BERT、LLaMA 都会把它堆叠几十层甚至上百层。本章先回答"为什么必须堆叠这么多层"，再引出 Transformer 真正学习的对象——**表示学习（Representation Learning）**，最后拆解不同深度的层各自倾向学到什么。理论讲完后，13.1 节会加载一个真实模型，把每一层的表示"挖"出来验证本章的结论。

---

## 为什么 Transformer 要堆叠很多层？

第11章拼出了一个完整的 Transformer Block，第12章又把它升级成使用 RoPE、RMSNorm、SwiGLU 和 GQA 的现代版本。但无论朴素版还是现代版，**单个** Block 的结构深度是一样的。那么为什么 GPT、BERT、LLaMA 都不是只用一个 Block，而要堆叠几十层甚至上百层？
...
## 表示学习（Representation Learning）——Transformer 真正学习的是什么？
...
但光看理论还不够——"不同层学到不同信息"这个结论是否真实存在于一个真正训练好的模型中？下一节，我们将加载一个开源小模型，亲手把每一层的表示"挖"出来验证这个结论：**13.1 Layer Probing——不同层表示的探测与分析**。
