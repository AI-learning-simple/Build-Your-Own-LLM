# 13. Stacking Transformer

> **本章目标**
>
> 第 11、12 章拼出了单个 Transformer Block（朴素版与现代版），但 GPT、BERT、LLaMA 都会把它堆叠几十层甚至上百层。本章先回答"为什么必须堆叠这么多层"，再引出 Transformer 真正学习的对象——**Representation（表示）**，最后拆解不同深度的层各自倾向学到什么。理论讲完后，13.1 节会加载一个真实模型，把每一层的表示"挖"出来验证本章的结论。
