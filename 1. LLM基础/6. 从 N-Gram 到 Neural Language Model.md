...
| 6.2 第一次前向 | `Linear → ReLU → Linear` | 第 4 章 4.5~4.7 | 把 MLP 接到真实 Context 输入 |
| 6.3 第一次预测 | Logits → 概率 | 第 4 章 4.8 | 第一次真正预测一个词 |
| 6.4 第一次打分 | `Loss = -log(P)` | 第 4 章 4.9 | 真实错误案例与 Perplexity |
| 6.5 | 反向传播：梯度从哪来 | 4.1 实践的数字链 | 亲手算出每个梯度 |
| 6.6 | 梯度下降：梯度怎么用 | 全新 | 用梯度更新参数 |
| 6.7 | 第一次端到端训练 | 前述全部环节 | 让模型真正学会 |
| 6.8 | Adam、权重初始化 | 全新 | 更稳定的训练工程 |
| 6.9 | 阶段性成果 | 第 1~6 章全部零件 | 训练第一个完整模型 |

其中，6.1~6.4 负责接通一次前向流水线；6.5~6.8 负责解释模型怎样从错误中学习；6.9 汇总阶段性成果。代码也会按这个顺序演进，每一步只增加一个环节。

---

# 6.0 N-Gram 统计表的问题：查过的才认识
