...
| 6.7 | 反向传播（梯度从哪来） | 全新（复用第4章数字链） | 亲手算出每个梯度 |
| 6.8 | 梯度下降（梯度怎么用） | 全新 | 模型如何学会 |
| 6.9 | 第一次端到端训练 | 全新 | 让模型真正学会 |
| 6.10 | Adam、权重初始化 | 全新 | 现代优化器 |
| 6.11 | 阶段性成果：训练 Embedding 模型 | 第1~6章全部零件 | 第一个完整模型（共现矩阵+SVD） |

也就是说：6.1~6.6 是"把已认识的零件装进同一条流水线"，从 6.7 起才是本章真正的新知识——且顺序与 Karpathy micrograd 一致：**先亲手算出梯度（6.7 反向传播），再用梯度训练（6.8 梯度下降 → 6.9 完整训练 → 6.10 优化器）**。代码演进的原则不变——每一步都只在前一步的基础上增加一个环节。
