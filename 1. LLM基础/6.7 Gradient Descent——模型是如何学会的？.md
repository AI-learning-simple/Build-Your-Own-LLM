...
对于简单函数（如 `(w-3)²`），我们可以手动求导。但神经网络有成千上万个参数，手动求导几乎不可能。现代深度学习框架（PyTorch、TensorFlow）都实现了**自动求导（Automatic Differentiation）**，通过**反向传播（Backpropagation）**，从输出的 Loss 开始，沿着计算图反向逐层计算每一个参数的梯度。我们会在 **6.10 节亲手展开这个过程**：用第4章的真实数字链手算一次完整反向传播，再写一个迷你自动求导引擎，与 PyTorch 三方对账。这里先记住核心结论：
