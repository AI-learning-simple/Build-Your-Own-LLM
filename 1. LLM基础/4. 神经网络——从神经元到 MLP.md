...
> **本章目标**
>
> 前三章已经完成了 `Text → Token IDs → Embedding`。但向量只负责表示，不能自己完成预测。本章从一颗神经元出发，依次理解 **线性层（Linear）、激活函数（Activation）、多层感知机（MLP）、未归一化分数（Logits）、Softmax 与交叉熵损失（Cross Entropy Loss）**，建立一条完整的前向传播理论链路。
>
...
## 五、线性层（Linear Layer）：用矩阵并行计算多个神经元
...
**这个公式的作用**：用一次矩阵运算并行计算整层神经元。输入是一个 `d_in` 维向量，输出是一个 `d_out` 维向量；矩阵 `W` 的每一行就是一个神经元的 Weight。

这里出现了一个贯穿全书的术语：**Shape（形状）**，指一个数组每个维度上有多少个元素。例如 `[0.2, -0.3, 0.5]` 的 Shape 是 `(3,)`，表示这是一个长度为 3 的向量；一个 4 行 3 列的矩阵 Shape 是 `(4, 3)`。代码里用 `x.shape` 就能查看。后面每接一个新模块，都要先确认 Shape 是否对得上。
...
解决办法是在 Linear 之间插入 **激活函数（Activation Function）**。

---

## 六、激活函数（Activation Function）：引入非线性
...
**这个公式的作用**：对每个输入元素分别执行“负数置 0、正数保留”，从而在两层 Linear 之间加入不能被矩阵乘法合并的非线性。几何上，一个 `ReLU(wx+b)` 会沿 $wx+b=0$ 形成一道“折痕”；多个神经元组合许多折痕，就能构造复杂的分段线性边界。
...
ReLU 只是最简单的一个。为了避免后续章节突然出现陌生名字，这里先集中列出本书会用到的几个，它们都是**逐元素**作用的一元函数：

| 名称 | 定义 | 值域 | 本书首次使用 |
|---|---|---|---|
| ReLU | `max(0, x)` | `[0, +∞)` | 第4章 |
| Sigmoid `σ` | `1 / (1 + e^(-x))` | `(0, 1)` | 第9章 LSTM 门控 |
| tanh | `(e^x − e^(−x)) / (e^x + e^(−x))` | `(−1, 1)` | 第9章 RNN Cell |
| GELU | 约等于 `x · Φ(x)`，`Φ` 是标准正态分布的累积分布函数 | 允许小幅负值 | 第11章 FFN |
| SiLU（又名 Swish） | `x · σ(x)` | 允许小幅负值 | 第12章 SwiGLU |

几个便于记忆的要点：

- **Sigmoid 输出 0～1，天然适合当"开关"或"通过比例"**，所以 LSTM 的三个门都用它；
- **tanh 输出 −1～1，且以 0 为中心**，适合表示可正可负的状态值；
- **GELU、SiLU 是 ReLU 的平滑版本**，在 0 附近不是硬折角，负区间也保留一小部分信号。

现代 Transformer 常用 GELU 或 SwiGLU。它们的具体形状不同，但核心作用与 ReLU 一致：**在 Linear 之间引入非线性**。ReLU 会把负区间全部压成 0，神经元可能长期失活；更平滑或带门控的激活可以缓解这一问题。

---

## 七、多层感知机（MLP）：Linear → Activation → Linear
...
两者都复用 `Linear → 非线性 → Linear` 的思想，但 FFN 的第二层不是词表输出层。第11、13章会进一步展开这个区别。

---

## 八、Softmax：把未归一化分数（Logits）变成概率

Logits 只是相对分数，可以为负，也不要求总和为 1。Language Model 的采样、Temperature、Top-k 与 Top-p 需要的是整个概率分布，因此要用 Softmax 做归一化。

对第 $i$ 个候选，Softmax 用它的 Logit 与全部候选共同计算概率：

```math
p_i=\frac{e^{z_i}}{\sum_{j=1}^{|V|}e^{z_j}}
```

**这个公式的作用**：把任意实数 Logits 转换成总和为 1 的概率分布。

- `z_i`：第 `i` 个 Token 的 Logit；
- `|V|`：Vocabulary 大小；
- 分母：所有候选指数值之和；
- `p_i`：第 `i` 个 Token 的预测概率。
...
## 九、交叉熵损失（Cross Entropy Loss）：衡量预测有多差
...
**这个公式的作用**：只读取真实答案位置的概率，并把它转换成一个“概率越低、惩罚越大”的标量 Loss。

- `t`：真实 Token 的类别编号；
- `p_t`：模型分配给真实 Token 的概率；
- `-log`：把接近 0 的概率放大成很大的错误值。
...
训练的直接目标不是让某个 Weight 单独变大或变小，而是调整所有可学习参数，使大量样本上的平均 Loss 下降。Weight、Bias 与 Embedding 都属于参数，会沿同一优化过程更新；第7章将用反向传播与梯度下降说明它们具体怎样改变。
...
- Loss 已经得到，Weight、Bias 与 Embedding 怎样修改才能让它下降？第7章将引入反向传播与梯度下降。
