...
刚刚设计的 Memory 实际上需要三个动作：**删除（Forget）**、**加入（Input）**、**输出（Output）**。1997 年提出的原始 LSTM 已经引入了可控的长期记忆通路和输入门、输出门；今天最常见的 **Forget Gate（遗忘门）** 是 Gers 等人在 1999 年提出、并在后续标准 LSTM 中广泛采用的改进。
...
本节说明了一个设计思路：Memory 不应该被无条件覆盖，而应该可控。由此推出三个需要的动作——**忘掉一部分旧信息、写入一部分新信息、决定这一步对外输出多少**，分别对应 Forget Gate、Input Gate 与 Output Gate。

---

# 下一节预告

三个门只是"需要什么"，还没有说明"怎么实现"。下一节 **9.8 LSTM——给 Memory 增加"长期记忆"** 会引入 Cell State，并说明这三个门具体如何作用在它上面。
