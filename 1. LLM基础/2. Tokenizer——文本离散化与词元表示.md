...
完成本章后，文本已经变成了一串整数。但这些整数只是**编号**，`3` 和 `4` 相邻不代表这两个 Token 含义相近，模型也无法对编号直接做加权求和。下一章 **第3章 Embedding——Token 的向量表示** 会解释：为什么 Token ID 不能直接输入神经网络，以及如何把每个编号换成一个可训练的向量。
