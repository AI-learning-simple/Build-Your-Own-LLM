...
1. **序列长度过长。** 一段普通文字可能只有几十个 Token，逐像素表示却有 50,176 个位置——Attention 的计算量随序列长度呈平方增长（第 15 章 Long Context 一节讲过这一点）。
...
**先交代一个背景：图像不是从来就这样处理的。** 在 ViT 出现之前，视觉的主流工具是 **CNN（卷积神经网络）**：用**卷积（Convolution）**——一个小窗口（如 3×3）在图像上滑动，每一步只提取窗口内的局部特征；再用**池化（Pooling）**——把相邻像素合并、缩小图像尺寸。两者反复堆叠，局部特征逐层组合成全局语义（边缘 → 纹理 → 物体部件 → 完整物体）。这套"滑动窗口"范式统治了计算机视觉十几年（ResNet 是其中的代表，第11章讲过的残差连接正是诞生于此）。

**ViT 做了一个更激进的选择：干脆不提特征，直接把图切块。** 卷积天生只看局部邻域（感受野要堆很多层才能覆盖全图），而 Attention 一步就能建立任意两个位置的全局关系；更关键的是，切完块之后，图像和文字就走上了**完全相同的流水线**（向量 → Transformer Block），模型结构零改动——这正是本章末尾那句话的伏笔：**Attention 从不关心输入是文字还是图像，它只关心向量**。这也顺带解释了为什么本书不讲卷积：**LLM 生态已经统一到 Transformer，卷积在现代多模态模型里已退化为历史背景**。下面就看这个"切块"具体怎么做。
...
**关键洞察：Attention 根本不关心输入是文字还是图像。** 回顾第 10 章的 Attention 公式 `Softmax(QKᵀ/√d)V`，它唯二关心的是 Query、Key、Value 这几个向量——不管这些向量最初是从文字 Token 来的，还是从图像 Patch 来的，Attention 的计算方式完全一样。因此 ViT 的做法极其直接：**把这些 Patch 向量，连同一个额外的 `[CLS]` 向量，整个塞进和第 11 章讲过的一模一样的 Transformer Block（Multi-Head Attention + FFN + Residual + LayerNorm）里，反复堆叠很多层**：

```mermaid
flowchart LR
    A["图片"] --> B["切成 N 个 Patch"] --> C["Patch Embedding<br/>(Linear 投影)"] --> D["+ [CLS] 向量"] --> E["Transformer Block × L 层<br/>(和第 11 章完全同一种 Block)"] --> F["图像的 Representation"]
```

堆叠很多层之后，每个 Patch 向量都在不断"关注"其它 Patch（这只狗的耳朵关注它的身体、天空关注地面……），逐渐提炼出越来越抽象的图像特征——这正是第 13 章讲过的"浅层学边缘纹理、深层学语义概念"的规律，在图像上同样成立。

**怎么训练这个 Vision Encoder，才能让它学到"有意义"的特征？** 目前用得最广泛的方法叫 **CLIP（Contrastive Language-Image Pretraining）**，思路和第 15 章的自监督学习一脉相承——不需要人工标注"这张图是猫"，而是利用互联网上天然大量存在的"图片 + 配文"数据对：

1. 一张图片经过 Vision Encoder，得到一个图像向量 `v_img`；配文文字经过第3、10、11章介绍的 Embedding、Attention 与 Transformer，得到一个文本向量 `v_text`。
2. 把一个 Batch 里所有图文对的向量两两做点积（第10.2节学过的 Dot Product，衡量"像不像"），得到一个相似度矩阵。
3. **这里直接复用了第10.3节的 Softmax 和第6.2 节的 Cross Entropy**：先对图文向量做 L2 归一化，再计算余弦相似度，并除以温度 $\tau$（CLIP 通常学习等价的 `logit_scale = 1/τ`）；缩放后的 logits 进入 Softmax 与 Cross Entropy。温度控制分布尖锐程度，而不是可省略的实现细节。
...
Q-Former 使用的是 **Cross-Attention（交叉注意力）**。它与第 10 章的 Self-Attention 使用同一套 `Softmax(QKᵀ/√d)V` 公式，但 Q、K、V 的来源不同：Self-Attention 的 Q/K/V 来自同一序列；Cross-Attention 中，Q 来自一组可学习 Query，K/V 来自图像 Patch 特征。Q-Former 通常准备若干个可学习 Query（例如32个），每个 Query 从全部 Patch 中加权提取信息，最终把较长的图像特征序列压缩成固定数量的向量。
...
经过 Vision Encoder + Projector，一张图片变成了 `N` 个和文字 Token 维度完全相同的"图像 Token"。剩下的事情非常简单：**把图像 Token 和文字 Token 拼接成同一条序列，直接送进第11、12章讲过的 Decoder-Only Transformer**：
...
flowchart LR
    ...
    F --> G
    G --> H["Decoder-Only Transformer<br/>(第11、12章)"] --> I["Next Token Prediction<br/>生成回答"]
```
...
至此，多模态大模型最核心的架构拼图已经完整了：**Patch Embedding 负责"切图"，Vision Encoder 负责"理解图"，Projector 负责"翻译"，剩下的一切都交给我们已经学过的 Transformer。** 这也是为什么第 10 章说过的那句话在这里依然成立：**Attention 从不关心数据来自哪里，它只关心向量。**
...
想亲手体验一次 Patch Embedding 和图文对齐是怎么算出来的，可以看这一节实验：**16.1 实现 Patch Embedding 与最小图文对齐模型**。
...
**第二步：多模态指令微调（Instruction Tuning）。** 数据换成"图片 + 问题 + 标准回答"的指令数据（如 LLaVA-Instruct），训练时**解冻 LLM（甚至 Vision Encoder），用全量微调或第二部分 4.2 节学过的 LoRA**——目标从"看得懂"变成"会回答"。这一步决定了模型"听话"的程度，和纯文本 LLM 的 SFT 完全同构。

**第三步：偏好优化（可选）。** 数据是人类对多模态回答的偏好排序，用第二部分 5.2 节的 DPO（或 RLHF）让模型学会"答得更好"——和纯文本 LLM 的偏好学习一模一样。
