# Experiment 6.10：亲手实现反向传播——从 Loss 到每一个 Weight 的梯度

> **本节目标**：6.7 节讲到梯度时，我们留下一句话——"反向传播的数学细节不属于本书重点"。现在，6.8 节我们用 PyTorch 完成了训练，6.9 节补上了优化器与权重初始化，是时候把这块最后的欠账亲手补上了。本节，我们将用第4章那条真实数字链，**手算一次完整的反向传播**，再用约 60 行代码写一个迷你自动求导（micrograd-lite），最后与 PyTorch 的 `loss.backward()` **三方对账**，证明框架背后做的正是同一件事。

---

# 一、为什么还要亲手算一遍？

6.8 节训练时，我们只写了一行 `loss.backward()`，梯度就自动出现了。但这一行背后到底发生了什么？

> **反向传播（Backpropagation）的本质，就是反复使用链式法则：从 Loss 出发，沿着"计算图"一层一层往回，把"Loss 对每一层输入的导数"乘出来。**

6.7 节我们用 `L(w) = (w-3)²` 这个单变量例子演示了"梯度是导数"。但真实模型是一张有向计算图：`x → Linear → ReLU → Linear → Softmax → Loss`。要算出 `dL/dW1`，必须经过中间每一层，把局部导数一路乘回去——这就是链式法则。
...
看起来简单，但"乘回去"的顺序和每一步乘什么，需要亲手走一遍才能真正理解。这也是为什么本节要用手算——**算完这一次，`loss.backward()` 对你就不再是黑盒**。
...
**`dL/dx` 是本节最值得停下来看的一个数。** 它回答的是第4章 4.10 节结尾留下的问题："`love` 的特征向量是不是也要跟着一起改？"答案是：**要，而且梯度就是它**。如果 `x` 不是手写的特征，而是 Embedding 查表的结果（第6.2节），这个梯度会沿着查表操作原样传回 Embedding Matrix 里被查到的那一行——这正是 6.7 节"Embedding 和 Weight 用同一条公式更新"这句话的完整证明。
...
下面这个迷你引擎（受 Karpathy 的 micrograd 启发，这里是向量版）只有 4 种运算：矩阵乘法、加法、ReLU，以及一个"Softmax + Cross Entropy"融合节点——**每一种运算的 backward，用的正是我们上面手算的公式**：
...
最后，用 PyTorch（6.8 节用过的那套 `nn.Linear` + `loss.backward()`）做第三份梯度，三份放一起逐元素比较：
...
回到 6.8 节那个训练循环，现在你可以把每一行都"翻译"出来了：

```python
logits = model(x)                    # 前向:计算图建立完成
loss = nn.functional.cross_entropy(...)
loss.backward()                      # 反向:沿着计算图逐层乘出每个参数的梯度
optimizer.step()                     # 6.9 节:AdamW 拿着梯度更新参数
```

`loss.backward()` 干的就是第五节到第八节我们干的事——只不过它用引擎化的代码、在 GPU 上、对几百万个参数同时完成。而 `optimizer.step()` 用的，正是 6.9 节手写的那个更新公式 `w = w - lr × m_hat / (√v_hat + eps)`。

到这里，第6章的拼图全部齐了：**前向传播（第4章 + 6.2~6.5）→ 损失（6.6）→ 梯度（6.7 + 本节）→ 优化（6.9）→ 完整训练（6.8）**。你亲手走过了从"一个随机的 MLP"到"一个会学习的语言模型"的每一步。
...
- Karpathy micrograd（约 150 行自动求导引擎，6.10 节迷你引擎的原型）：https://github.com/karpathy/micrograd
