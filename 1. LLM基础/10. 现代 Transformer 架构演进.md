...
|---|---|---|
| 位置信息 | 可学习绝对位置向量（第9章已说明，9.1节用于实践） | RoPE（旋转位置编码） |
...
偶数维使用正弦，奇数维使用余弦，不同维度对应不同频率；按上式即可直接生成位置编码。10.1 节将转而实现现代模型更常用的 RoPE。
...
RoPE 做的事情就是：**Token 位于哪个位置，就把它的 Q、K 向量按该位置对应的角度旋转**。实际模型把向量维度两两分组，每组使用不同频率；位置越靠后，累计旋转角度越大。第10.1节会把这组二维旋转公式扩展为可运行的多维实现。
...
回顾第9章的 LayerNorm：对一个 Token 的向量先减均值，再除以标准差，最后乘可学习缩放参数并加偏置。
...
其中 `g` 是可学习的逐维缩放参数。完整 PyTorch 实现放在 10.1 节。
...
`W_gate` 生成门控值，`W_up` 生成内容，两者逐元素相乘后再由 `W_down` 投影回模型维度。完整模块实现放在 10.1 节。
...
自回归生成时，模型会缓存历史 Token 的 Key 和 Value，避免每生成一个新 Token 都从头重复计算；这部分缓存叫 **KV Cache**。在标准 Multi-Head Attention 中，各个 Head 拥有独立的 K、V，序列越长、Head 越多，缓存占用越大。**GQA 和 MQA 通过共享 K、V 来压缩这部分显存。** 第13.4节会进一步实测 KV Cache 对显存和速度的影响。
