# Experiment 6.8：Train Your First Neural Language Model

## 实验目标

6.1～6.6 已经用 NumPy 拆解了 Embedding Lookup、Concatenate、MLP、Softmax、Loss、反向传播与参数更新，6.7 又解释了 AdamW 和权重初始化。本实验开始使用 **PyTorch**，把这些组件整合成一个完整、可训练的 Neural Language Model，并观察 Loss 如何随训练下降。
...
criterion = nn.CrossEntropyLoss()  # 对应 6.4
optimizer = torch.optim.AdamW(     # 对应 6.7
    model.parameters(),
    lr=0.01,
    weight_decay=1e-4,
)
...
`nn.CrossEntropyLoss()` 接收原始 Logits，并在内部组合数值稳定的 `log_softmax` 与负对数似然；不要先手动 Softmax。`AdamW` 使用反向传播得到的梯度更新参数，并把权重衰减与自适应梯度更新解耦。这个玩具数据集不需要复杂调度器；6.9 会在更大语料上加入 Warmup 与 Cosine Decay。
...
从 6.1 到 6.8，我们依次接通了 Neural Language Model 的完整训练链：数据集构建、Embedding Lookup、Concatenate（6.1）→ MLP Forward（6.2）→ Softmax（6.3）→ Cross Entropy（6.4）→ 反向传播（6.5）→ Gradient Descent（6.6）→ AdamW 与初始化（6.7）→ PyTorch 端到端训练（本节）。
...
从早期 Neural Language Model 到现代 GPT，训练主线始终包含前向计算、损失函数、反向传播和参数更新，但网络结构、优化器和工程规模不断演进。本节在玩具数据上验证了完整训练循环；下一节 6.9 将把同一结构扩展到 WikiText-2 英文语料，加入独立验证集、Perplexity、Warmup 与 Cosine Decay，并检查学到的 Embedding。
