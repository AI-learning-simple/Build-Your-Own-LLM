# 9.4 Unrolling——一个 RNN Cell 如何组成整个 RNN？
...
# Experiment 9.4：验证 Parameter Sharing
...
hidden = np.zeros(2)
tokens = [np.array([1,0]), np.array([0,1]), np.array([1,1])]  # I, love, AI（与 9.2、9.3 节相同）
...
# Experiment 9.5：手动展开 RNN（Unrolling）
