...
Loss 是单个标量，但模型包含 Embedding、`W1`、`b1`、`W2`、`b2` 等许多参数。对任一参数 $\theta$，局部导数

$$
\frac{\partial L}{\partial \theta}
$$

表示参数发生微小变化时，Loss 在当前位置如何变化。标量参数只有一个导数；向量和矩阵参数的梯度与参数 Shape 相同。因此 `W1.shape == W1_grad.shape`，优化器才能执行逐元素更新。
...
因此，**学习率不是梯度，也不是 Loss**。梯度来自反向传播；学习率是训练配置中的超参数，用来缩放梯度形成实际更新步长。若梯度为 `-6`：
...
7.1 节手算梯度时，只喂了**一条样本**（`[love, AI] → today`，拼接输入为 6 维）。但真实训练每一步会同时喂一批样本（比如 32 条），Loss 是这一批样本各自 Loss 的**平均值**：
