# Experiment 6.7：亲手实现反向传播——从 Loss 到每一个 Weight 的梯度

> **本节目标**：6.1~6.6 节，我们把前向流水线一步步搭了起来——数据（6.1）、Embedding（6.2）、拼接（6.3）、MLP 前向（6.4）、Softmax（6.5）、Loss（6.6）。模型终于能"知道自己错了"，但一个关键问题还没回答：**Loss 对每一个 Weight 的梯度，到底是怎么算出来的？** 不知道梯度，就谈不上"调整参数"。本节按照 Karpathy 在 micrograd 里教的方法，先用第4章那条真实数字链**手算一次完整的反向传播**，再用约 60 行代码写一个迷你自动求导引擎（micrograd-lite），最后与 PyTorch 的 `loss.backward()` **三方对账**——算完这一次，6.8 的梯度下降和 6.9 的完整训练对你就不再是黑盒。

---

# 一、为什么还要亲手算一遍？

等 6.9 节真正训练时，我们只会写一行 `loss.backward()`，梯度就自动出现了。但这一行背后到底发生了什么？本节先把它揭开。

> **反向传播（Backpropagation）的本质，就是反复使用链式法则：从 Loss 出发，沿着"计算图"一层一层往回，把"Loss 对每一层输入的导数"乘出来。**

梯度就是导数。但真实模型不是单变量函数，而是一张有向计算图：`x → Linear → ReLU → Linear → Softmax → Loss`。要算出 `dL/dW1`，必须经过中间每一层，把局部导数一路乘回去——这就是链式法则。
...
## 💡 矩阵乘法的两条导数规律（Karpathy nanoGPT 视频的"转置直觉"）
...
**`dL/dx` 是本节最值得停下来看的一个数。** 它回答的是第4章 4.10 节结尾留下的问题："`love` 的特征向量是不是也要跟着一起改？"答案是：**要，而且梯度就是它**。如果 `x` 不是手写的特征，而是 Embedding 查表的结果（第6.2节），这个梯度会沿着查表操作原样传回 Embedding Matrix 里被查到的那一行——这正是 6.8 节"Embedding 和 Weight 用同一条公式更新"这句话的完整证明。
...
最后，用 PyTorch（6.9 节训练时会用到的那套 `nn.Linear` + `loss.backward()`）做第三份梯度，三份放一起逐元素比较：
...
回到 6.9 节那个训练循环，现在你可以把每一行都"翻译"出来了：

```python
logits = model(x)                    # 前向:计算图建立完成
loss = nn.functional.cross_entropy(...)
loss.backward()                      # 反向:沿着计算图逐层乘出每个参数的梯度
optimizer.step()                     # 6.10 节:AdamW 拿着梯度更新参数
```

`loss.backward()` 干的就是第五节到第八节我们干的事——只不过它用引擎化的代码、在 GPU 上、对几百万个参数同时完成。而 `optimizer.step()` 用的，正是 6.10 节手写的那个更新公式 `w = w - lr × m_hat / (√v_hat + eps)`。

到这里，第6章的拼图全部齐了：**前向传播（第4章 + 6.2~6.5）→ 损失（6.6）→ 梯度（本节，反向传播）→ 梯度下降（6.8）→ 完整训练（6.9）→ 现代优化器（6.10）**。你亲手走过了从"一个随机的 MLP"到"一个会学习的语言模型"的每一步。
