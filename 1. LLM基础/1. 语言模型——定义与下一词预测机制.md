...
假设让你完成一句话：`I love ______`，你可能会想到 `AI`、`Python`、`music`、`cats` 等答案。这个例子可以帮助我们直观理解条件概率：前面的内容会改变后续词出现的可能性。它只是语言模型训练目标的类比，不等于把人类语言活动全部解释为下一词预测。
...
**Model（模型）**：一个能够从数据中估计规律，并根据规律进行预测的数学模型。它可以非常简单（例如统计 `hello` 后面出现 `world` 的频率），也可以非常复杂（例如包含大量可学习参数的 Transformer），但这里讨论的共同目标都是预测下一个 Token。
...
最后再从这些概率中选择一个 Token 进行输出。**怎么选**属于推理阶段的**解码策略（Decoding Strategy）**：每次都取概率最高的那个叫 Greedy；按概率随机抽取叫 Sampling；Top-k、Temperature 则用来控制随机的范围和程度。第4章会先讲清概率是怎么算出来的，11.1 生成文字时会第一次用到 Greedy 与 Sampling，13.3 再在真实模型上调整 Temperature。本章只需要记住：

> **Language Model 输出的是一个概率分布，而不是一句完整的话。**

---

## 三、Training 与 Inference
...
在模型上下文窗口与能力允许的范围内，相关 Context 往往有助于预测；无关内容也可能形成干扰。模型如何聚合上下文信息，会在第 10 章 Attention 和第 11 章 Transformer Block 中继续讨论。

---

## 五、Autoregressive（自回归）
...
**第一章到这里结束。**下一部分将进入 **实验 1.1：Character Bigram Language Model**。从这一节开始，我们将亲手写代码，构建整本书的第一个语言模型，并且这份代码会一直演进，最终成长为一个完整的 nanoGPT。
