...
最后再从这些概率中选择一个 Token 进行输出。**怎么选**属于推理阶段的**解码策略（Decoding Strategy）**：每次都取概率最高的那个叫 Greedy；按概率随机抽取叫 Sampling；Top-k、Temperature 则用来控制随机的范围和程度。第4章会先讲清概率是怎么算出来的，11.1 生成文字时会第一次用到 Greedy 与 Sampling，15.3 再在真实模型上调整 Temperature。本章只需要记住：
