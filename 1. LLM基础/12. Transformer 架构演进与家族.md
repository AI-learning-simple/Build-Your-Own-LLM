...
## 方案四：相对位置编码与 ALiBi——把"距离"直接加进 Attention Score

前三种方案都在**输入端或 Q/K 上**做文章：往 Token Embedding 上加向量（方案一、二），或者旋转 Q、K（方案三）。方案四换了一个作用点——**输入不动、Q/K 也不动，而是直接给 Attention Score 加一个只与"两个位置相隔多远"有关的偏置项**：

```math
\operatorname{Attention}(Q,K,V)=\operatorname{Softmax}\left(\frac{QK^{\top}}{\sqrt{d_k}}+B\right)V,\qquad B_{ij}=b_{\,i-j}
```

**这个公式的作用**：让注意力分数的形状不再只取决于"两个 Token 内容像不像"，还取决于"它们离多远"——距离相同，加上的偏置就相同。

- `B`：位置偏置矩阵，形状 `(T, T)`，只由下标之差 `i-j` 决定，因此天然表达的是**相对**距离；
- `b_{i-j}`：距离 `i-j` 对应的一个标量。它可以是**可学习**的一组参数，也可以是**固定公式**；
- 可学习的代表是 **T5 的相对位置分桶偏置**：把距离归到若干桶里（近处一桶一档、远处合并成几档），每个桶、每个 Head 学一个标量；
- 固定公式的代表是 **ALiBi（Attention with Linear Biases）**，直接让分数随距离线性衰减：

```math
b_{\,i-j}^{\,(h)}=-m_h\cdot(i-j),\qquad i\ge j
```

- `m_h`：第 `h` 个 Head 的斜率，是**固定常数、不参与训练**，通常按几何级数取（例如 8 个 Head 依次为 `1/2, 1/4, …, 1/256`）。不同 Head 衰减快慢不同，于是有的 Head 主要看近邻、有的 Head 还能看到很远；
- `i-j`：Query 位置 `i` 与 Key 位置 `j` 的距离，`j>i`（未来位置）由 Causal Mask 直接屏蔽。

数值上很直观：设 `m=0.5`，距离 `i-j=0` 时偏置为 `0`（不惩罚），距离 `2` 时偏置为 `-1`，距离 `4` 时偏置为 `-2`。也就是"离得越远，越容易被 Softmax 压低"，而这个惩罚是**显式写出来的**，不是从旋转相位里间接体现的。

**优点**：

- 不占用 Embedding 维度、不新增位置表，位置信息只在 Score 上"加一笔"；
- 相对距离是显式建模的：距离相同、偏置相同，模型不必自己从绝对位置里反推远近；
- ALiBi 的斜率由公式给定，超出训练长度后偏置仍能按距离自然延续，因此在长度外推上通常比方案一更稳。

**缺点**：

- 偏置形式是**人为设计的先验**：分桶边界、最大距离、斜率序列都要人来定，比"直接学一张位置表"多了调参；
- 只压 Score，不改变 Q/K 本身的表示，和 RoPE 这类几何变换的表达方式不同；
- ALiBi 能自然延续偏置，但"训练 2K、推理 32K 不掉点"依然不成立——外推表现仍取决于训练配置与评测方式。

**代表模型**：T5 系列使用可学习的相对位置分桶偏置；BLOOM 使用 ALiBi。今天的主流开源大模型大多已转向 RoPE。

---

## 四种方案放在一起看

| 方案 | 位置信息加在哪里 | 是否新增参数 | 绝对 / 相对 | 长度外推 | 代表模型 |
| --- | --- | --- | --- | --- | --- |
| 一、绝对位置编码（可学习） | 输入端：加到 Token Embedding | 有，`block_size × d_model` | 绝对 | 差，超过位置表行数直接越界 | GPT-2、GPT-3、11.1 节的 nanoGPT |
| 二、正弦位置编码 | 输入端：加到 Token Embedding | 无，固定公式 | 绝对（可由三角函数推出相对位移） | 可计算任意位置，但训练长度之外的表现仍需验证 | 2017 原始 Transformer |
| 三、RoPE | Q / K 上：按位置旋转 | 无，固定旋转 | 相对（点积显式依赖 `i-j`） | 角度仍随绝对位置累积，需插值 / NTK / YaRN 等扩展 | Llama、Qwen、Mistral、DeepSeek |
| 四、相对位置偏置 / ALiBi | Attention Score 上：加偏置 | T5 可分桶学习；ALiBi 无 | 相对 | 靠固定线性衰减，相对较稳，但非无条件 | T5、BLOOM |

一句话记住它们的区别：**方案一、二改输入，方案三改 Q/K，方案四改 Score**；作用点越靠后，相对距离就表达得越"显式"。
...
✅ 不含位置信息的 Self-Attention 对 Token 排列是等变的，必须额外注入顺序 ✅ 位置编码有四类：可学习绝对向量、正弦、RoPE、相对偏置 / ALiBi，作用点分别在输入、输入、Q/K 与 Attention Score ✅ 正弦位置编码可计算任意位置，但可靠外推仍需验证 ✅ RoPE 把相对位移引入 Q/K 点积，但不自动保证长度外推 ✅ RMSNorm 省略中心化，SwiGLU 引入门控分支；实际效果依赖训练配置 ✅ GQA/MQA 通过减少 KV Head 数，按比例压缩 KV Cache 的理论主体占用 ✅ Transformer 演化出 Encoder / Decoder / Encoder-Decoder 三种架构 ✅ Encoder 擅长理解、Decoder 擅长生成、Encoder-Decoder 擅长信息变换 ✅ LLM 选择 Decoder Only，是因为训练目标与生成任务高度一致，且更容易利用海量无标注数据
