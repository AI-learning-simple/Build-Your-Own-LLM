...
**如果把多层网络的权重全部初始化为 0 会怎样？** 单独看到某一层输出为 0，还不足以证明它不能学习；必须检查反向梯度。下面使用两个无 Bias 的 Linear 层，并把两层权重都清零：
...
loss.backward()
print(model[0].weight.grad.norm().item())  # 0.0
print(model[2].weight.grad.norm().item())  # 0.0
```

第二层的输入为全 0，所以第二层权重梯度为 0；第二层权重本身又为 0，传回第一层的梯度也为 0。这个具体网络会卡住。更一般地，同一隐藏层若以完全相同的参数开始并始终接收相同梯度，就无法打破神经元置换对称性。随机初始化用于打破这种对称性；但“任意单层权重为 0 都绝对不能学习”并不成立，结论必须结合网络结构。
...
- **同时观察 train loss 和 val loss**（9.1 节会演示）：train loss 反映优化是否进行，val loss 用于评估未见数据上的泛化；两者在随机小 Batch 下都可能短期波动；
...
到这里，Neural Language Model 主线已经覆盖数据准备与前向（6.1～6.4）、反向传播（6.5）、梯度下降（6.6）、端到端训练（6.7）和现代优化器（本节）。下一节（6.9）是一个独立的静态词向量旁支：用共现统计、PPMI 和 SVD 构建语义空间；它不是把本章反向传播与优化器再次组装成一个模型。之后再进入 RNN，继续序列建模主线。
