...
| 模型        | 词表大小 (vocab\_size) | 对应 num\_merges |
| :-------- | :----------------: | :------------- |
| **GPT-2** |       ~50,000      | 约 50,000 次合并   |
| **BERT**  |       ~30,000      | 约 30,000 次合并   |
| **T5**    |       ~32,000      | 约 32,000 次合并   |

注意：实际实现中通常直接设 vocab_size（目标词表大小），而不是手动数 num_merges。因为初始词表有 256 个单字节，所以 num_merges ≈ vocab_size - 256。
经验法则：让大多数常见词变成 1~2 个 token，生僻词拆成 3~5 个 token。
num_merges 通常 = 目标词表大小 - 初始字符数（GPT-2 约 5 万）。核心原则是：高频组合合并，低频组合保留拆分，让模型既能认识常见词，又能拼出生僻词。
