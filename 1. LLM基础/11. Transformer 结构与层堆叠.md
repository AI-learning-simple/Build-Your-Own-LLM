...
Transformer Block 已经拥有 Multi-Head Attention、FFN、Residual、LayerNorm，模型已经非常完整。但训练时仍会遇到第 8 章定义过的现象：**模型在训练集上表现很好，换到没见过的数据上却突然变差**——这叫做 **Overfitting（过拟合）**，本质是"记住答案，而不是学会规律"。

第 8 章是从数据与评估的角度认识过拟合：用验证集监控 train/val 两条曲线、必要时早停。这一节则要回答第 8 章没展开的问题——**在神经网络内部，过拟合是怎么发生的？**

答案是：模型中的某些神经元会过度依赖另一些特定的神经元，形成一种只对训练数据有效的"惯性组合"。例如某三个神经元总是一起激活，只要凑齐这个组合，模型就直接给出训练时见过的答案，而不是重新推理。参数量足够大时，模型完全有能力把训练数据里的每个细节都"背"进这样的组合里——这正是第 8 章"记住答案"在网络内部的具体形态。
...
这里要再分清两个概念：**序列长度固定** 和 **每个位置的上下文长度固定**。在 GPT 训练中，通常把一个 batch 里多个样本统一截成同一长度 `T`（例如 256），这说明一条样本的长度固定为 256，不是说每个位置都只能看 256 个 token。对位置 `j` 来说，它看到的是前 `j+1` 个 token，并被要求预测它右边紧接着的 token：
...
也就是说，**`x` 与 `y` 整体右移一位**；模型在每个位置上都在学习“给定左侧上下文，预测下一个 token”。这种设计让一个窗口中的每个位置都成为一个监督点，同时保留了并行训练的效率。
