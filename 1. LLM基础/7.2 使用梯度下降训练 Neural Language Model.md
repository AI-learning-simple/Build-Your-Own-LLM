# 7.2 使用梯度下降训练神经语言模型（Neural Language Model）

> **硬件要求**：CPU 即可运行（普通笔记本可跑）

## 实验目标

第6章已经接通 `Embedding → Concatenate → MLP → Loss`，第7章和 7.1 又解释了反向传播与参数更新。本实验使用 **PyTorch** 把这些组件整合成一个可训练的神经语言模型（Neural Language Model），并使用最基础的随机梯度下降（SGD）观察损失（Loss）如何随训练下降。AdamW 与学习率调度留到第8章，避免在解释之前把现代优化器当作黑盒。
...
**这个公式的作用**：给出完全均匀随机预测时的 Cross Entropy 基线，其中 `V` 是 Vocabulary Size。它用于检查 Loss 量级，不是训练必须达到的固定初值。
