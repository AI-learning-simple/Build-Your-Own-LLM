...
这里有一个容易混淆的点：**一个 batch 里的序列长度固定**，并不等于**每个位置的有效上下文长度都固定**。在训练时，通常会把多个样本或多个窗口拼成同一长度 `T`（例如 `block_size=256`），这表示一条样本的长度是 256，而不是说第 0 号位置只看 256 个 token、第 1 号位置也只看 256 个 token。实际上，位置 `j` 能看到的是它左侧的历史上下文；如果输入序列是 `x = [t_0, t_1, ..., t_{T-1}]`，那么位置 `j` 处理的上下文长度是 `j+1`，它的训练目标是预测 `t_{j+1}`。因此在 GPT 的训练里，常见写法是：
...
也就是 **`x` 与 `y` 整体右移一位**，每个位置都在预测“下一个 token”。

Encoder 的双向 Self-Attention 通常允许每个 Query 查看整段序列，称为 **Full Attention**。Decoder-only 语言模型则在训练时加入 **Causal Mask**，让位置 `i` 只能查看 `j≤i` 的 Key，避免它在计算时偷看未来位置。两者的区别只在 Mask：Full Attention 没有屏蔽位置，Causal Attention 的严格上三角权重为 0。这样一来，模型可以并行计算整段序列，但每个位置都仍然遵守“只能根据过去预测未来”的因果约束。
...
把这四步的形状演变画成图（三维魔方式：X = 特征维、Y = Token 维 T、Z = 批次/头维）——`d_model` 被切成 `H` 段，头被 transpose 提到批次维旁，最后 `B` 与 `H` 折叠成一柱：

![Multi-Head Attention 的形状演变：拆分多头后把头折叠进批次](./assets/multi_head_batch_pack_diagram.png)

生成脚本见 [`multi_head_batch_pack_diagram.py`](./assets/multi_head_batch_pack_diagram.py)。
