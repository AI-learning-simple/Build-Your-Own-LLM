...
> **💡 单位矩阵投影 = 不投影（实验 10.2 的技巧，这里先讲清楚）**：如果令 $W_Q = W_K = W_V = I$（单位矩阵），则 $Q = XI = X$——投影后还是原来的向量，Q、K、V 退化成前面简化版里的"直接等于 Token Embedding"。**实验里把三个投影矩阵设为单位阵，不是为了模拟真实模型，而是为了隔离变量**：先不看投影的效果，把"缩放点积 + Softmax + Mask"本身算清楚；等机制都看明白了，再把投影换回可学习的 $W_Q/W_K/W_V$（训练中由数据学出来的数字）。这也是所有实验的标准做法——一次只改变一个变量。
