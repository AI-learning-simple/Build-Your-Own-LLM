...
**第二步：多模态指令微调（Instruction Tuning）。** 数据换成"图片 + 问题 + 标准回答"的指令数据（如 LLaVA-Instruct），训练时**解冻 LLM（甚至 Vision Encoder），用全量微调或第13.3节学过的 LoRA**——目标从"看得懂"变成"会回答"。这一步决定了模型"听话"的程度，和纯文本 LLM 的 SFT 完全同构。

**第三步：偏好优化（可选）。** 数据是人类对多模态回答的偏好排序，用第13.4节的 DPO（或 RLHF）让模型学会"答得更好"——和纯文本 LLM 的偏好学习一模一样。
