...
3. **这里直接复用了第8.3节的 Softmax 和第6.4节的 Cross Entropy**：希望"配对正确"的那一对（图片和它真正的配文）相似度最高，Softmax 之后概率最大，用 Cross Entropy 计算 Loss、反向传播。
...
✅ 图片不能直接喂给 Transformer，原因和第2、3章讲过的"字符太细碎、One-Hot 维度太高"是同一类问题的图像版本 ✅ Patch Embedding 把图片切成小块、用 Linear 投影成向量，相当于图像世界的"Tokenizer + Embedding" ✅ Vision Encoder 就是把 Patch 向量塞进和第9章完全相同的 Transformer Block，Attention 不关心输入来自文字还是图像 ✅ CLIP 用对比学习训练 Vision Encoder，复用的正是第8.3节 Softmax 和第6.4节 Cross Entropy ✅ Projector 是一层（或几层）Linear，负责把图像向量"翻译"到 LLM 的 Embedding 空间，通常是多模态模型里唯一真正需要重新训练的部分 ✅ 图像 Token 和文字 Token 拼接成同一条序列后，剩下的生成过程和纯文本 LLM 完全一样
