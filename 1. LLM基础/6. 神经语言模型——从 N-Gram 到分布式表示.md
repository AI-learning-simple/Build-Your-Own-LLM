...
第5章已经给出过这个术语的定义，这里补充它为什么无法靠"多收集语料"解决。假设词表大小为 $V$，固定窗口包含 $N$ 个 Token，仅理论上的 Context 组合就可能达到 $V^N$ 种。随着窗口变长，组合数呈指数增长，而训练语料只能覆盖其中很小的一部分。
...
本章结束时，模型已经可以从 Context 算出概率和 Loss，但参数仍然停留在随机初始值——它知道自己错了，却不知道该怎么改。下一章 **第7章 反向传播与梯度下降** 会解释 Loss 如何变成每个参数的梯度，以及参数如何据此更新。
