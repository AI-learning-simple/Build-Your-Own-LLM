...
第 4 章已经用玩具例子介绍了神经网络的零件，本章要把它们装进真正的语言模型流水线。各节分工如下：

| 本节 | 流水线位置 | 概念出处 | 本节新增 |
|---|---|---|---|
| 本章入口 | N-Gram → Neural LM | 第 5 章 N-Gram | 数据稀疏、泛化与整体结构 |
| 6.1 Concatenate | 数据集 → Embedding → 拼接输入 | 第 2、3、5 章 | 在实验准备后，用拼接保留词序 |
| 6.2 第一次前向 | `Linear → ReLU → Linear` | 第 4 章 Linear、Activation 与 MLP | 把 MLP 接到真实 Context 输入 |
| 6.3 第一次预测 | Logits → 概率 | 第 4 章 Softmax | 第一次真正预测一个词 |
| 6.4 第一次打分 | `Loss = -log(P)` | 第 4 章 Cross Entropy Loss | 真实错误案例与 Perplexity |
| 6.5 | 反向传播：梯度从哪来 | 4.1 实践的数字链 | 亲手算出每个梯度 |
| 6.6 | 梯度下降：梯度怎么用 | 全新 | 用梯度更新参数 |
| 6.7 | AdamW、学习率与初始化 | 6.6 参数更新 | 建立现代训练方法 |
| 6.8 | 第一次端到端训练 | 前述全部环节 | 用 PyTorch 训练玩具模型 |
| 6.9 | 阶段性成果 | 第 1~6 章全部零件 | 在 WikiText-2 上训练模型 |

其中，6.1～6.4 接通一次前向流水线，6.5～6.7 依次解释梯度计算、参数更新和现代优化器，6.8 用 PyTorch 完成第一次端到端训练，6.9 再把同一方法扩展到真实英文语料。
...
下一节从实验准备开始，把 `[I, love] → AI` 与 `[love, AI] → today` 转成 Token ID 样本，并选择第二条样本接通 `Embedding → Concatenate`，为 6.2 的 MLP 前向计算准备同一个 6 维输入。
