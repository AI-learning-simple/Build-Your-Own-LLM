# Experiment 6.7：从 Vanilla Gradient Descent 到 Adam——现代优化器与权重初始化

## 实验目标

6.6 节已经建立了最朴素的参数更新公式 `w = w - lr × grad`。本节沿着这条公式继续推导 Momentum、Adam 与 AdamW，并解释 Warmup、学习率衰减和权重初始化。先理解这些机制，下一节才能在完整训练代码中明确 `optimizer` 为什么这样选择，而不是把 Adam 当作未经解释的黑盒。
...
到这里，Neural Language Model 主线已经覆盖数据准备与前向（6.1～6.4）、反向传播（6.5）、梯度下降（6.6）和现代优化器（本节）。下一节 6.8 将使用 PyTorch 完成第一次端到端训练；6.9 再把同一方法扩展到 WikiText-2 英文语料，并检查验证集指标和 Embedding 结构。
