# 9.3 RNN Cell——Hidden State 是如何更新的？
...
tokens = [np.array([1,0]), np.array([0,1]), np.array([1,1])]  # I, love, AI（与9.2 节相同）
...
每一次输出都不同，因为 Hidden State 一直在变化——但注意，这里已经不是 9.2 节的简单衰减，而是真正可学习的 `tanh(Wx·token + Wh·hidden + b)`。
