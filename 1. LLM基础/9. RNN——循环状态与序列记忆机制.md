...
## tanh 与 Sigmoid：两个 S 形激活函数怎么分工？

第4章的表格提到过，本书会用到 Sigmoid 和 tanh 这两个"S 形"激活函数：本章的 RNN Cell 用 tanh；再往后第十一～十三节的 LSTM 门控会用 Sigmoid。它们长得很像（都是把任意实数压缩到一个有限区间），但分工完全不同，提前放在一起看清楚，后面遇到 Sigmoid 就不会陌生：

![Sigmoid vs tanh：两种 S 形激活函数，分工完全不同](./assets/tanh_sigmoid_curves.png)

| | Sigmoid `σ(x) = 1/(1+e^(-x))` | tanh `(e^x−e^(-x))/(e^x+e^(-x))` |
|---|---|---|
| 值域 | `(0, 1)` | `(-1, 1)`，以 0 为中心 |
| x = 0 时输出 | `0.5` | `0` |
| 适合表达的语义 | "比例、开关"——0 表示完全关闭，1 表示完全打开 | "可正可负的状态值"——需要表示"往哪个方向变化" |
| 本书用在哪 | LSTM 的三个门：Forget / Input / Output（见第十一～十三节） | RNN Cell 的 Hidden State 更新；LSTM 候选记忆 `C̃ₜ` |

**为什么 Hidden State 更新用 tanh，不用 Sigmoid？** 因为 Hidden State 每一维代表一种"特征强度"，往往需要表示"正向证据"或"负向证据"（例如某一维表示情感倾向，需要能取负值表示消极）。如果换成 Sigmoid，输出永远是正数，Hidden State 就丢掉了表示"负向"的能力。反过来，Sigmoid 输出恰好落在 `(0, 1)`，天生适合表示"保留多少比例、放行多少比例"，这正是后面 Forget/Input/Output Gate 需要的语义——门控关心的是"通过比例"，不是"正负方向"。

一句话记住：**tanh 用来表示"值"，Sigmoid 用来表示"比例"。**
...
光看符号容易觉得抽象，代入一组具体数字走一遍，公式就不再神秘：

![RNN Cell 单步计算：一个具体的数值例子](./assets/rnn_cell_numeric_step.png)

处理 Token "AI" 时，`x_t=[2.00, 1.50]`、`h_{t-1}=[0.80, -0.60]`，先做 Linear 得到 `z=[1.23, 1.26]`——两个数都已经超出 `(-1, 1)`；再过 `tanh`，被压回 `h_t=[0.84, 0.85]`。下面柱状图里能直接看到：紫色（`z`）比蓝色（`h_t`）更"冲出边界"，tanh 就是把这种可能无限增长的数值，重新框回状态该有的范围。
...
既然 RNN Cell 是"一层神经元"，它和第 4 章的 MLP 层到底差在哪？先说相同点：两者都是**线性变换 + bias + 激活函数**，都由神经元组成，都是一个"把输入向量变成输出向量"的层，并且——像所有神经网络一样——这一层的权重在训练好之后，每次被调用时用的都是**同一套** `W、b`（这一点 MLP、RNN、后面的 Transformer 都一样，并不是 RNN 独有的性质）。

真正的区别在**输入来源**：

| | MLP 层（第 4 章） | RNN Cell 层（本章） |
|---|---|---|
| 输入来源 | 只有前一层传来的特征 | 当前 Token `xₜ` + 上一时刻自己的输出 `hₜ₋₁` |
| 计算公式 | `y = activation(W·x + b)` | `hₜ = tanh(Wxh·xₜ + Whh·hₜ₋₁ + b)` |
| 是否有反馈回路 | 没有，数据只单向往后走 | 有，`hₜ` 会被送回来参与下一次计算 |
| 如何变深 | 堆叠更多层 | 堆叠层（深度方向）+ 沿时间展开（时间方向） |

真正让 RNN 特殊的，是**这条"输出接回输入"的反馈回路**：`hₜ` 算出来之后不会消失，而是被重新喂回同一层，去参与下一步 `hₜ₊₁` 的计算。正因为有了这条回路，`h₂` 才会带着 `h₁`（也就是 Token1）的信息，`h₃` 才会带着 `h₁、h₂` 两步的信息——**记忆是回路产生的，不是参数本身产生的**。MLP 没有这条回路，所以第 1 层的输出交给第 2 层之后，第 1 层不会再被同一个输入调用第二次。

这里有一个最重要的洞察：**把 RNN 沿时间展开，就等价于一个深度 = 序列长度的前馈网络，只是每往后展开一步，用的都是同一层（同一套 `Wxh、Whh、b`），而且这一层的输入里额外混入了上一步的输出。** 换句话说，RNN 可以看成"把同一个 MLP 层反复调用、层与层之间再接一条反馈线，沿时间轴铺开"。第五节会继续说明：这套权重不但在每次调用时相同，更重要的是**它不会因为当前处理的是第几个 Token 而改变**——这才是 RNN 真正想强调的"参数共享"。
...
这里要强调一下"共享"具体共享的是什么，避免和"MLP 的参数也是固定的"混淆。第 6 章的固定窗口模型（回顾 6.1 的 Concatenate）里，`window_size=2` 时输入是 `[love 向量, AI 向量]` 拼接成的 6 维向量，`W1` 这个矩阵的前 3 列专门对接"窗口里第 1 个位置"，后 3 列专门对接"窗口里第 2 个位置"——**同一个 Token，出现在窗口的不同位置，会乘上不同的权重列块**。而 RNN 处理 `xₜ` 时，不管 `t` 是 1 还是 100，永远是同一套 `Wxh、Whh` 在起作用，**不存在"专门负责第 3 个位置"的权重**。

RNN 最重要的特点就是：**所有 Time Step，都用同一套参数处理当前输入，参数不会因为"这是序列中第几个 Token"而改变。** 这就是 **Parameter Sharing（参数共享）**——它共享的是"跨序列位置"，而不是在说"MLP 不共享参数"（MLP 每层的参数当然也是固定复用的，只是它并没有"同一套权重要在不同位置反复承担同一角色"这个场景）。

为什么 RNN 必须这样共享？固定窗口模型的 `W1` 的形状是 `(hidden_size, window_size × embedding_dim)`，天生绑定了窗口大小；如果 RNN 也像它那样给每个 Time Step 单独分配一套权重，一句 100 Token 的话就需要 100 套 Weight，另一句 200 Token 的话需要 200 套 Weight，模型根本无法处理不同长度的句子。共享同一套 `Wxh、Whh、b`，才让 RNN 的参数数量与序列长度脱钩。

用真实数字跑一遍 `I love AI today` 这四个 Token，能更直观地看到"同一套参数、Hidden 在变"：

![Unrolling 数值示例：Hidden State 随 Token 演变](./assets/rnn_hidden_state_evolution.png)

左边热力图里，`Wxh、Whh、b` 从头到尾只有一份，四列（`h1~h4`）颜色深浅不同，是因为**输入在变、Hidden 在变，而不是参数在变**；右边折线图更能看出"记忆"是怎么积累的——每一步的曲线走向都同时受"当前 Token"和"上一步的位置"影响，`h4` 里已经隐含了 `h1、h2、h3` 一路传下来的信息，不是只看最后一个 Token `today` 算出来的。
...
## RNN 全流程图：把前面几节的零件拼成一整条链

到这里，Token、Hidden State、激活函数、输出投影这几个零件都已经分别讲过，很容易散落成一堆孤立的知识点。下面把它们按照真实的计算顺序，沿时间轴连续拼接两步，看一次完整的流程（以 Many-to-Many 同步输出为例，`h₀` 是初始 Hidden State，通常取全 0 向量）：

```mermaid
flowchart LR
    H0["h₀（初始 Hidden，常为 0）"] --> L1["Linear<br/>Wxh·x₁+Whh·h₀+b"]
    X1["x₁：Token 1"] --> L1
    L1 --> A1["tanh 激活函数"]
    A1 --> H1["h₁：新 Hidden State"]
    H1 --> P1["输出投影<br/>Wy·h₁+by"]
    P1 --> S1["Softmax"]
    S1 --> Y1["预测 y₁"]
    H1 --> L2["Linear<br/>Wxh·x₂+Whh·h₁+b"]
    X2["x₂：Token 2"] --> L2
    L2 --> A2["tanh 激活函数"]
    A2 --> H2["h₂：新 Hidden State"]
    H2 --> P2["输出投影"]
    P2 --> S2["Softmax"]
    S2 --> Y2["预测 y₂"]
    H2 -.->|"同一套 Wxh/Whh/b<br/>继续沿时间展开"| L3["..."]
```

顺着这张图从左到右走一遍，正好把本章前六节串起来：

1. **`h₀ + x₁ → Linear`**：第四节的 `Wxh·xₜ + Whh·hₜ₋₁ + b`，两路输入（当前 Token、上一时刻 Hidden）先各自加权再相加；
2. **`Linear → tanh → h₁`**：第四节讲的激活函数，把可能超出范围的线性结果压回 `(-1, 1)`，得到新的 Hidden State——这一步就是"记忆被更新"的瞬间；
3. **`h₁ → 输出投影 → Softmax → 预测 y₁`**：第七节的输出策略，Hidden State 本身不是最终答案，还要再过一层线性变换（词表大小的 `Wy`）和 Softmax，才能得到"预测下一个词的概率分布"；这一步是否存在、什么时候执行，取决于任务是 Many-to-One 还是 Many-to-Many；
4. **`h₁` 同时被送往下一个 Time Step**：这就是第四节说的"反馈回路"——`h₁` 一边去算 `y₁`，一边被喂回同一个 RNN Cell，和 `x₂` 一起算出 `h₂`；
5. **第二步用的 `Wxh、Whh、b` 和第一步完全是同一套**：这正是第五节的 Parameter Sharing，图里没有出现"第二个 Cell"，只是同一个计算反复执行。

一句话总结整张图：**输入永远是"当前 Token + 上一步 Hidden”，输出永远来自"当前 Hidden 再过一层投影”，而串起这一切、让记忆能跨步传递的，是 Hidden State 这条从 `h₀` 一路延伸下去的链。**
...
把"连乘"这件事画成图，差距有多夸张一眼就能看出来：

![BPTT 的核心风险：反复乘同一个 Jacobian，指数级放大或缩小](./assets/bptt_gradient_scaling.png)

三条曲线用的是同一个道理（简化成标量系数 `r`，真实场景下 `r` 是 Jacobian 矩阵的量级），只是 `r` 取值不同：`r=0.5` 时梯度按指数**衰减**，回传 10 步后只剩 `0.5¹⁰≈0.001`；`r=1.3` 时梯度按指数**爆炸**，回传 10 步后放大到 `1.3¹⁰≈13.8`；只有 `r=1.0` 时才能一直稳定传下去。序列越长，这种指数级差距就越夸张——这也是为什么"梯度消失/爆炸"几乎是所有较长序列 RNN 都会遇到的问题，而不是偶然的训练失误。
...
其中 `σ`（Sigmoid）保证输出一定在 0~1 之间，`Wf`、`bf` 是需要学习的参数。因此 Forget Gate 不是程序员写死的固定值，而是模型训练出来的——什么时候应该接近 1，什么时候应该接近 0。这里终于用上了第四节里提前对比过的 Sigmoid：Hidden State 用 tanh 是因为需要表示"值"，Forget Gate 用 Sigmoid 是因为它只需要表示"比例"——保留多少、忘掉多少，天生就该落在 `(0, 1)`。
...
把 Forget、Input、Output 三个门串起来，用"Alice、教授、北京大学、天气很好"这个笔记本例子走一遍完整数值，正好对应第十一～十三节的全部公式：

![LSTM 三个门的完整数值示例](./assets/lstm_gates_numeric.png)

- **左图（Forget + Input）**：`f_t` 对 `Alice、教授、北京大学` 接近 `1`（几乎全保留），对`天气很好`只有 `0.05`（几乎全部忘掉）；这一步暂时没有值得写入的重要新信息，所以 `i_t` 和候选内容 `C̃_t` 普遍偏低。
- **中图（Cell State 更新）**：套入 `Cₜ = fₜ⊙Cₜ₋₁ + iₜ⊙C̃ₜ`，前三条数值几乎不变，`天气很好`从 `0.75` 直接掉到 `0.05` 附近——这就是"遗忘"在数字上的样子。
- **右图（Output Gate）**：假设此刻的问题是"Alice 在哪里工作？"，`o_t` 在"北京大学"这一维给到 `0.90`，其余维度都很低；套入 `hₜ = oₜ × tanh(Cₜ)` 后，`h_t` 里数值最大的正是"北京大学"——Cell State 里其它信息（`Alice、教授`）依然完整保留，只是这一刻没有被输出。
...
## LSTM 全流程图：一个 Time Step 内部到底发生了什么

前面三节把 Forget、Input、Output 三个门分开讲，容易让人误以为它们是三个先后执行的步骤。实际上，三个门在同一个 Time Step 内**并行计算**，只是最后汇合的方式不同。下面把整条链路画在一张图里：

```mermaid
flowchart LR
    Hprev["h_t-1"] --> Concat["拼接 [h_t-1, x_t]"]
    Xt["x_t：当前 Token"] --> Concat
    Concat --> F["Forget Gate<br/>f_t = σ(Wf·[h,x]+bf)"]
    Concat --> I["Input Gate<br/>i_t = σ(Wi·[h,x]+bi)"]
    Concat --> Cand["候选记忆<br/>C̃t = tanh(Wc·[h,x]+bc)"]
    Concat --> O["Output Gate<br/>o_t = σ(Wo·[h,x]+bo)"]
    Cprev["C_t-1（旧 Cell State）"] --> Mul1["逐元素乘"]
    F --> Mul1
    I --> Mul2["逐元素乘"]
    Cand --> Mul2
    Mul1 --> Add["相加"]
    Mul2 --> Add
    Add --> Ct["C_t：新 Cell State"]
    Ct --> TanhC["tanh 压缩"]
    TanhC --> Mul3["逐元素乘"]
    O --> Mul3
    Mul3 --> Ht["h_t：新 Hidden State（也是当前输出）"]
    Ct -.->|"传给下一时刻"| Cnext["C_t+1 ..."]
    Ht -.->|"传给下一时刻"| Hnext["h_t+1 ..."]
```

按图里的分支梳理一遍数据流向：

1. **输入拼接**：`h_t-1` 和 `x_t` 先拼在一起，作为四个门/候选记忆共同的输入——这也是为什么四条公式长得很像，区别只在各自的权重矩阵（`Wf、Wi、Wc、Wo`）和最后用的激活函数；
2. **左边三路（Forget、Input、候选记忆）汇入 Cell State 更新**：`f_t` 先决定"旧 Cell State 留多少"（`Mul1`），`i_t` 决定"候选记忆写多少"（`Mul2`），两路相加得到新的 `C_t`——这正是第十一、十二节的 `Cₜ = fₜ⊙Cₜ₋₁ + iₜ⊙C̃ₜ`；
3. **右边一路（Output Gate）汇入 Hidden State 输出**：新的 `C_t` 先过 `tanh` 压缩到 `(-1,1)`，再和 `o_t` 逐元素相乘，得到 `h_t`——对应第十三节的 `hₜ = oₜ × tanh(Cₜ)`；
4. **两条通道分别向右延伸到下一个 Time Step**：`C_t` 走的是**长期记忆通道**，几乎不经过压缩直接传递（图中上面那条横线）；`h_t` 走的是**短期工作通道**，每一步都要重新经过 `tanh` 和 Output Gate 的过滤——这正是第十节说的"两种 Memory"在图上的样子。

对比这张图和上一节 RNN 的全流程图，能看出 LSTM 到底多了什么：RNN 只有一条 `h` 链，LSTM 在这条 `h` 链之外，**又加了一条几乎不衰减的 `C` 链**，并且用三个 Sigmoid 门控制这条新链和旧链之间的信息交换——这就是 LSTM 相对 RNN 唯二的结构性变化（多一条状态、多三个门），除此之外，输入方式、参数共享、沿时间展开这些机制和 RNN 完全一样。
...
- ✅ tanh 与 Sigmoid 的分工：tanh 表示"值"（可正可负），Sigmoid 表示"比例"（0~1）
- ✅ RNN Cell 层和 MLP 层的区别与联系
- ✅ Unrolling 与 Parameter Sharing（时间方向）
- ✅ 多层 RNN / Stacked RNN（深度方向）
- ✅ RNN 的四种输出方式
- ✅ RNN 全流程图：Token → Linear → tanh → Hidden State → 输出投影 → Softmax，以及 Hidden State 如何反馈进下一步
...
- ✅ LSTM 全流程图：三个门并行计算，Cell State 走长期通道、Hidden State 走短期通道
