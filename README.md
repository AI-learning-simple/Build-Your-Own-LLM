...
| 6 | 从 N-Gram 到 Neural Language Model | N-Gram 数据稀疏性、Neural LM 核心思想、Cross Entropy → Perplexity、反向传播（手算 + micrograd）、Gradient Descent → Adam/AdamW、权重初始化、**6.11 阶段性成果：亲手训练 Embedding 模型（共现矩阵 + PPMI + SVD，king - man + woman ≈ queen）** |
