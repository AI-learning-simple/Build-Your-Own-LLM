# 第15章 综合实战——从零构建一个能写进简历的文档问答 Agent
...
| 服务层 | 用 FastAPI 把能力暴露成 HTTP API | 第10章 Agent Runtime |
...
| "怎么上线？" | 加 Docker 打包、vLLM 部署（第8章）、加日志与可观测性（第10章） |

**可以继续扩展的方向**（把它从"作品"升级成"能聊半小时的项目"）：把检索层换成真实 Embedding 模型 + FAISS；接入第2章的 Tool Calling，让 Agent 在"检索"之外还能"调计算器、查天气"；给服务加流式输出（第10.2节）；加多轮对话（复用第4章 Memory）。
