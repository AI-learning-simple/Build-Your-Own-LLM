# 第 8 章 Agent 协议家族——MCP、A2A、AG-UI 分别连接了什么？
...
**MCP 最大的价值之一是能力可以被自动发现**：Client 连接 Server 后，可以调用 `list_tools()` 获取 Server 提供的全部工具及其参数定义，不需要事先在代码里硬编码"这个 Server 有哪些工具"。8.1 节的实验里，我们亲手验证了这一点：写好的 MCP Server 一旦启动，Client 立即能自动列出全部工具，完全不需要人工维护工具清单。
...
想亲手搭建一个真实的 MCP Server，并让 Agent 通过标准协议调用它，可以看这一节实验：**8.1亲手搭建一个 MCP Server，并让 Agent 通过 MCP 协议调用工具**。想接着体验 A2A 协议下两个 Agent 如何互相声明能力并协作，可以看：**8.2 使用 A2A 协议实现两个 Agent 之间的协作**。
...
> **第 9 章 Skill——Agent 如何拥有可复用的专业能力**
