...
Alignment 决定了模型"应该表现成什么样子"，但真正让这一切在工业级规模上跑起来，离不开另一套工程体系，而这套工程体系又建立在对GPU 硬件本身的理解之上：

```mermaid
flowchart LR
    G[GPU 基础与算子] --> A[LLM Training Stack] --> C[训练出模型]
    G --> B[LLM Inference Stack] --> D[服务好用户]
    C --> E[LLM Evaluation]
    D --> E
    E --> F[持续迭代]
```

- **GPU 基础与算子**（SM/显存层级/算子融合）解释了"为什么显存会成为瓶颈""为什么 FlashAttention 更快"，是理解后面所有工程方案的地基；
...
| 章节 | 核心问题 | 关键概念 |
|---|---|---|
| 1 Alignment | 为什么 GPT 需要变成 ChatGPT | Base Model、行为对齐 |
| 2 SFT | 模型如何学会理解指令 | Chat Template、Loss Mask、LoRA |
| 3 Preference Learning | 模型如何学会人类偏好 | Reward Model、RLHF、DPO |
| 4 Reasoning Alignment | 模型如何学会真正推理 | Process/Outcome Supervision、GRPO |
| 5 Safety Alignment | 如何让模型既强大又安全 | Jailbreak、Guardrail、多层防护 |
| 6 GPU 基础与算子 | GPU 硬件与显存为何决定性能 | SM/Tensor Core、HBM/SRAM、算子融合 |
| 7 Training Stack | 如何训练超大模型 | 并行策略、ZeRO |
| 8 Inference Stack | 如何高效服务大量用户 | Continuous Batching、PagedAttention |
| 9 Evaluation | 如何科学评价模型 | Benchmark、LLM-as-a-Judge |
