...
第7.1节的实验里，我们即使只用一张 8GB 显卡，也体验了 ZeRO Stage 2 + CPU Offload 带来的真实显存降低——这正是因为 ZeRO 的思想同样能在单卡场景下，把 Optimizer 状态转移到 CPU 内存。
...
即使只有一张 8GB 显卡，也可以亲手体验 Accelerate 与 DeepSpeed ZeRO 的分布式训练配置，看这一节实验：**7.1 亲手用 Accelerate + DeepSpeed ZeRO 体验分布式训练配置**。
