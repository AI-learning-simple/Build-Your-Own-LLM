...
**Checkpoint 是最后一个容易被忽视的工程点**：分布式训练动辄数天，任何一张卡故障都可能毁掉全部进度，所以要么频繁保存全量 Checkpoint（几十 GB 到 TB 级），要么用异步保存 + 定期重启的长任务管理（Kubernetes 上的 Job 管理），要么用第 11 章（第三部分）讲过的 Checkpoint/Resume 思想——**训练系统首先得是一个可靠的系统，其次才是快的系统**。
