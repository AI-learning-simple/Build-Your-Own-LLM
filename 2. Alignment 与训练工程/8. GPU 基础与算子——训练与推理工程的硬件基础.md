...
问题就出在反复的"写入 HBM → 读出 HBM"：当序列长度是 N，这个 N×N 的中间矩阵可能非常大（例如 N=4096 时就是 1600 万个元素，FP16 下占 32MB），而每一步的实际计算量相对于"把这么大一块数据在 HBM 里来回搬运"而言并不算多——**这正是上面提到的 Memory-Bound：算力没有被用满，时间大部分花在了搬数据上**。更糟的是，这个 N×N 矩阵还要占一份额外的显存，序列一长，显存直接爆炸（这也是第一部分第14章“序列长度增长”的另一个侧面）。
