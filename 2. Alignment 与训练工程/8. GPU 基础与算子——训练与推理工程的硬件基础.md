...
**算子融合（Kernel Fusion）**，就是把多个连续的小算子合并成一个大算子，让中间结果直接留在 SRAM 里传递给下一步计算，不必经过 HBM 这一趟"远路"，同时也少了几次 Kernel Launch 的固定开销。`torch.compile`、Triton、XLA 这些"编译加速"技术，做的核心工作之一正是自动发现并融合可以合并的算子。（第8.1节会用 Triton 亲手写一个融合算子。）
...
问题就出在反复的"写入 HBM → 读出 HBM"：当序列长度是 N，这个 N×N 的中间矩阵可能非常大（例如 N=4096 时就是 1600 万个元素，FP16 下占 32MB），而每一步的实际计算量相对于"把这么大一块数据在 HBM 里来回搬运"而言并不算多——**这正是上面提到的 Memory-Bound：算力没有被用满，时间大部分花在了搬数据上**。更糟的是，这个 N×N 矩阵还要占一份额外的显存，序列一长，显存直接爆炸（这也是第一部分第16章“序列长度增长”的另一个侧面）。
...
关键在于：**每一层的误差不是独立噪声，而是被当作下一层的"真实输入"，参与下一次矩阵乘法和非线性变换。** 这和第11章 RNN 反向传播里"重复乘同一个 Jacobian 导致梯度消失/爆炸"是同一个数学结构——只是这里换成了前向传播、连乘的对象换成了每一层的（近似）线性变换。误差在每一层都可能被权重矩阵放大或缩小一点，只要平均放大系数略大于 1，经过几十层之后，初始的 `1e-7` 就会像滚雪球一样长到 `1e-3` 量级。这也是为什么**误差随深度呈指数增长的规律**，与网络是否收敛、是否用了残差连接关系不大——残差 + LayerNorm 能延缓这个过程（上面实验已经加了这两项），但不能从根本上消除它。
...
4. 为什么单层网络里 NumPy 和 PyTorch 的数值差异可以忽略，但堆到 100 多层后差异会显著放大？这和第11章 RNN 的梯度消失/爆炸有什么共同的数学结构？
