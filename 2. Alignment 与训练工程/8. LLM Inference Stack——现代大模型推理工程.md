...
这套“分页 + 页表”设计能够减少按最大长度预留造成的内部浪费，并回收请求结束后的物理页。实际显存利用率取决于页大小、请求长度分布、调度器和实现，不能把某个基准中的百分比当作普遍保证。PagedAttention 是 vLLM 的核心贡献，也是"推理引擎借鉴操作系统"的经典案例（和第三部分第 11 章 Graph Engineering 借鉴数据库思想异曲同工）。
