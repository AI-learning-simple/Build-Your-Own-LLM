...
第一部分 13.4 节讲过 KV Cache 避免重复计算，但它的代价是显存。一个并发请求的 KV Cache 显存大约为：
...
以 NVIDIA 的 TensorRT-LLM 为例，它和 vLLM 的架构一一对应：vLLM 的 Continuous Batching 在 TensorRT-LLM 里叫 **In-flight Batching（在途批处理）**，PagedAttention 对应它的 **Paged KV Cache**，两者是同一思想的两种工程实现；而 TensorRT-LLM 的杀手锏是 **FP8 量化**——它在 H100/Blackwell 的 Transformer Engine（第6章讲过）上把权重和 KV Cache 都压到 FP8，推理吞吐比 FP16 再上一个台阶。**换句话说，第8章的技术栈没有"流派之分"：vLLM 和 TensorRT-LLM 在概念层面收敛到了同一套方案，差异只在工程实现与硬件绑定深度。**
