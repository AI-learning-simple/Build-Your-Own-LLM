# 第 8 章 LLM Inference Stack——现代大模型推理工程
...
第 7 章的训练系统追求"用最短时间训练完一个模型"；推理系统面对的是完全不同的场景：**成千上万个用户同时发来长度各不相同的请求，系统需要在可接受的延迟下，同时服务尽可能多的用户**。两者的工程目标几乎是反的：
...
第一部分 15.4 节讲过 KV Cache 避免重复计算，但它的代价是显存。一个并发请求的 KV Cache 显存大约为：
...
这套"分页 + 页表"的设计，让 KV Cache 的显存利用率从 60%~70%（连续分配）提升到 99% 以上——**相当于在不加显存的情况下，多服务了几十个百分点的并发请求**。PagedAttention 是 vLLM 的核心贡献，也是"推理引擎借鉴操作系统"的经典案例（和第三部分第 11 章 Graph Engineering 借鉴数据库思想异曲同工）。
...
| 张量并行推理 | 模型权重拆到多卡（第 7 章思想），KV Cache 也分摊 | 装下超显存模型、并发翻倍 | 卡间通信开销，需要 NVLink |
| GQA / MLA | 让多个头共享 KV（第一部分第 12 章），或把 KV 压缩成低秩形式 | KV Cache 减少 4~8 倍 | 需要模型本身支持（预训练决定） |
...
以 NVIDIA 的 TensorRT-LLM 为例，它和 vLLM 的架构一一对应：vLLM 的 Continuous Batching 在 TensorRT-LLM 里叫 **In-flight Batching（在途批处理）**，PagedAttention 对应它的 **Paged KV Cache**，两者是同一思想的两种工程实现；而 TensorRT-LLM 的杀手锏是 **FP8 量化**——它在 H100/Blackwell 的 Transformer Engine（第6章讲过）上把权重和 KV Cache 都压到 FP8，推理吞吐比 FP16 再上一个台阶。**换句话说，第 8 章的技术栈没有"流派之分"：vLLM 和 TensorRT-LLM 在概念层面收敛到了同一套方案，差异只在工程实现与硬件绑定深度。**

---

想亲手测量 Continuous Batching 到底能带来多大的吞吐提升，可以看这一节实验：**8.1 使用 vLLM 验证 Continuous Batching——测量多请求并发下的吞吐提升**。
...
> **第 9 章 LLM Evaluation——如何科学评价一个大语言模型**
