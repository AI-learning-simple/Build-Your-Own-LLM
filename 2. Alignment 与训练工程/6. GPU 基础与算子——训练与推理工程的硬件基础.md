...
| 层级 | 全称 | 容量 | 速度 | 类比 |
|---|---|---|---|---|
| HBM（显存） | High Bandwidth Memory | 大（消费卡 8~24GB，数据中心卡 80~192GB） | 相对慢（带宽几百 GB/s 到几 TB/s） | 仓库：能放很多东西，但每次搬运都要走一段路 |
...
这引出了判断程序性能瓶颈的经典工具——**Roofline 模型**：一个程序的实际速度，取决于它是被"算力"卡住，还是被"数据搬运带宽"卡住。
...
**朴素实现的 Self-Attention 恰好是典型的 Memory-Bound 操作**——这一点会在本章最后和 6.1 节的实验里具体展开，这也是理解 FlashAttention 为什么有效的关键。
...
| 优化器状态（Adam） | ~420GB | 每个参数要存 FP32 主权重 + 一阶矩 + 二阶矩 = 12 字节 |
| 激活值（中间结果） | 数十到数百 GB | 取决于 batch size 与序列长度 |
| **合计** | **>700GB** | 远超任何单卡（A100/H100 也只有 80GB） |
...
✅ GPU 用数千个简单核心并行处理规整的数值运算，天生契合矩阵乘法 ✅ SM、CUDA Core、Tensor Core、Warp 是 GPU 从"车间"到"班组"的分工结构 ✅ HBM（大而慢）与 SRAM（小而快）两级存储，加上 Roofline 模型，可以判断程序是 Compute-Bound 还是 Memory-Bound ✅ 一张卡装得下什么由显存决定，搬得快不快由带宽决定，算得完不完整由算力决定——三者的真实数字各不相同 ✅ 训练显存账本：优化器状态 > 参数 > 梯度，激活值随 batch 暴涨——这是 ZeRO 与梯度检查点的出发点 ✅ 算子是深度学习的基本计算单元，算子融合通过减少 HBM 读写与 Kernel Launch 次数提速 ✅ CUDA 生态从 Driver 到 cuBLAS/cuDNN/NCCL 再到 PyTorch 层层封装，ROCm 是 AMD 对应生态 ✅ FlashAttention 不改变数学结果，仅靠重排计算顺序与数据搬运，就带来数量级的速度提升与显存节省
