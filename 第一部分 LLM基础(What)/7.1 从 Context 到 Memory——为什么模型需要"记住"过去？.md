# 7.1 从 Context 到 Memory——为什么模型需要"记住"过去？
...
上一章的 Neural Language Model 每次预测都是 `Context → MLP → Prediction`，预测结束后模型什么都不会留下，下一次预测又重新开始，完全没有连续性。

一个新的想法出现了：**每处理一个 Token，就把当前信息保存到Memory，供下一步使用**。这正是 RNN 的核心思想。

---

# Experiment 7.1：手写一个最简单的 Memory
