...
以后所有 Tokenizer 都放到 `tokenizer.py` 里面。
...
这比散落的几个函数更清晰，以后 WordTokenizer、BPETokenizer、SentencePieceTokenizer 都可以采用同样的接口。

---

# 三、实现 build()

Tokenizer 的第一件事情是建立 Vocabulary：
...
总代码**不到 30 行**，但已经完成了第一个 Tokenizer。
...
生成结束以后：`generated_text = tokenizer.decode(generated_ids)`。除此之外，**Language Model 一行不用改**。这就是 Tokenizer 最大的价值。

---

# 九、本实验修改了什么？

相比第一章，新增了 `CharacterTokenizer`、`encode()`/`decode()`、`Vocabulary`，而 Language Model **完全没动**。整个系统升级为：

```mermaid
flowchart LR
    A[Text] --> B[CharacterTokenizer] --> C["Token IDs"] --> D[Language Model] --> E["Generated IDs"] --> F[CharacterTokenizer] --> G[Text]
```
