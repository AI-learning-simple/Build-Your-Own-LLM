...
正是因为 LoRA/QLoRA 的出现，普通开发者用一张消费级显卡就可以微调一个数十亿参数的模型——这也是本书第12.3节 LoRA 实验以及后面全部微调实验能够在 8GB 显存内完成的根本原因。
...
想亲手拆开一条真实的 SFT 数据，看清楚 Chat Template 和 Loss Mask 到底是如何构造出来的，可以看这一节实验：**15.1 亲手构建一份 SFT 数据集——Chat Template 与 Loss Mask 实战**。
