# 实验 2.3：Unknown Token（`<UNK>`）

> **实验目标**
>
> 上一节，我们实现了 Word Tokenizer。它比 Character Tokenizer 更高效，但暴露出一个致命问题：**遇到词表中不存在的新词时，程序会直接崩溃**。本实验将引入 NLP 中第一个特殊 Token：**`<UNK>`（Unknown Token）**。
...
程序直接报错：`KeyError: 'ChatGPT'`。原因很简单，训练数据中只有 `I / love / AI`，Vocabulary 为 `{AI:0, I:1, love:2}`，当 Tokenizer 遇到 `ChatGPT` 时，它不知道应该映射成哪个 Token ID。这就是 **Out-of-Vocabulary（OOV）**，也就是词表之外的单词。
...
训练 `tokenizer = WordTokenizer("I love AI")`，Vocabulary 为 `{<UNK>:0, AI:1, I:2, love:3}`。编码`tokenizer.encode("I love ChatGPT")` 输出 `[2,3,0]`：`I→2`，`love→3`，`ChatGPT→<UNK>→0`。程序终于不会崩溃了。

---

# 六、再测试一句话

输入 `I love OpenAI`、`I love Claude`、`I love Gemini`，编码结果全部都是 `[2,3,0]`。你会发现所有未知词全部变成 `0`。
...
因此，虽然 Word Tokenizer + `<UNK>` 已经能够正常运行，但它无法处理**无限增长的新词**。现代 LLM 几乎都已经放弃 `<UNK>`，而采用 **Subword**——把 `ChatGPT` 切成 `Chat` + `GPT`，即使整个词没见过，模型也认识 `Chat` 和 `GPT`，于是自然能够理解 `ChatGPT`。
