# 7.5 RNN 的输出方式——Many-to-One、One-to-Many 与 Many-to-Many
...
例如**图片描述（Image Caption）**：输入一张图片，模型不断生成 Token：`Image → Hidden → A → cat → is → sleeping`。这里只有一个输入，后面不断生成新的 Token，这就是 One-to-Many。

**大语言模型的文本生成属于这一类**：用户输入一句话，模型不断生成新的 Token，直到结束。
