...
一个生活中的例子：假设你第一次阅读一本小说：第一遍只知道有哪些人物；第二遍开始理解人物之间的关系；第三遍发现很多伏笔；第四遍终于理解作者真正想表达什么。每读一遍，你看到的都是同一本书，但理解越来越深。Transformer 也是一样。

**每一层都在重新理解一句话。** 输入 `The cat is sleeping.`：第一层可能学到词性、主谓关系、修饰关系；第二层开始理解动作、主体、状态；第三层进一步理解整个句子的含义。请注意，不是第一层完成任务后第二层重复，而是每一层都站在前一层理解的基础上继续加工——这是一个**不断抽象**的过程。

例如输入 `Apple`：Embedding 只是一个词；第一层知道这是名词；第二层结合上下文知道这里表示公司；第三层知道 Apple 和 iPhone 存在产品关系；第四层知道这是一场新品发布会；第五层知道用户正在讨论科技新闻。请注意，每一层都没有改变 Token，而是在不断丰富它的语义表示。

**每一层到底学什么？** 研究人员分析 BERT、GPT、LLaMA 不同层，发现了一个有趣的规律：浅层更关注词形、拼写、局部语法；中层开始学习语义、实体关系、句法结构；深层更关注推理、任务目标、最终预测。这并不是严格分工，而是一种普遍观察到的趋势。

```mermaid
flowchart TD
    L1["Layer 1<br/>词法（Lexical）"] --> L4["Layer 4<br/>句法（Syntax）"] --> L8["Layer 8<br/>语义（Semantic）"] --> L16["Layer 16<br/>知识（Knowledge）"] --> L32["Layer 32<br/>推理（Reasoning）"]
```

请注意，真正的大模型每一层都会参与所有任务，这张图只是帮助理解不同深度通常承担不同层次的信息加工。
...
**本节小结**：✅ 一个 Transformer Block 已经可以理解上下文 ✅ 多层 Block 的价值在于不断深化表示 ✅ 每一层都会重新加工 Token 的语义表示 ✅ Transformer 本质上是一种层次化表示学习模型。一句话总结：**Transformer 不是靠某一层变聪明，而是靠一层一层不断深化理解。**
...
现在我们已经理解了为什么需要几十层 Transformer。接下来，我们将回答一个很多人都会疑惑的问题：Transformer 论文提出的是 `Encoder + Decoder`，为什么今天的大语言模型却几乎全部变成了 `Decoder Only`？Encoder 去哪了？下一章，我们正式进入 **Transformer 家族**。
