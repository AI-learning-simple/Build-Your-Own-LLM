...
极少数模型（如 Meta 的 Llama Guard 系列）在 Hugging Face 上被设置为需要单独申请访问权限的 Gated Model，这一步申请流程本身也需要能访问 `huggingface.co`。如果你暂时无法完成申请，不用担心——书中涉及 Gated Model 的章节（如第二部分 5.1）都额外提供了**不依赖任何 Gated Model 的替代方案**，直接跳过申请这一步，用替代方案完成实验即可。
