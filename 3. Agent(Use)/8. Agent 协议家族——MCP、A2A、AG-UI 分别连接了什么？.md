# 第8章 Agent 协议家族——MCP、A2A、AG-UI 分别连接了什么？
...
想亲手搭建一个真实的 MCP Server，并让 Agent 通过标准协议调用它，可以看这一节实验：**8.1亲手搭建一个 MCP Server，并让 Agent 通过 MCP 协议调用工具**。想接着体验 A2A 协议下两个 Agent 如何互相声明能力并协作，可以看：**8.2 亲手用 A2A 协议实现两个 Agent 之间的协作**。
