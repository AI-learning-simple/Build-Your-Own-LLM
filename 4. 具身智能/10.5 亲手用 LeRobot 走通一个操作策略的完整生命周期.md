# Experiment 10.5：亲手用 LeRobot 走通一个操作策略的完整生命周期
...
- `policy_preprocessor` / `policy_postprocessor` 是**数据归一化器**：训练时数据被归一化到标准范围，部署时输入也必须走同一套归一化，输出的动作还要反归一化回真实物理单位——这正是第 6.4 节"训练和推理必须保持一致"的工程化体现，**部署时漏掉它们，策略会直接失效**；
...
| 归一化/反归一化预处理 | 第 6.4 节（训练与推理一致性） |
| 断点续训、检查点管理 | 第 6.7~6.9 节（训练工程） |
